...
# I changed the https, but that turned out not to be necessar
```{r}
url <- "https://raw.githubusercontent.com/HAN-M3DM-Data-Mining/data-mining-s2y2122-KelvinWardenaar/master/datasets/NB-fakenews.csv"
rawDF <- read_csv(url)
View(rawDF)
```
...
# I did not change anything on this part of the code
```{r}
str(rawDF)
head(rawDF)
```
...
# I changed the dimensions from 20000 to 6000
```{r}
...
# Changed it from testCorpus to trainCorpus
```{r}
...
# Switched around the 1 and 0 ("1,0")
```{r}
# building a function 
...
# Changed the colomn from type to label 
```{r}
